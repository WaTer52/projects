...
@Mapper
public interface Yac001QueryDao {
    ...
    @Select("select * from Yac001 where `departure`=#{departure} and `arrival`=#{arrival} and `departureDate`=#{departureDate} and `seatNum` >= #{needs}")
