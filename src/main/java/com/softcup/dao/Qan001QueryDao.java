...
@Mapper
public interface Qan001QueryDao {
    ...
    @Select("select * from Qan001 where `departure`=#{departure} and `arrival`=#{arrival} and `departureDate`=#{departureDate} and `seatNum` >= #{needs}")
