...
@Mapper
public interface Sha001QueryDao {
    ...
    @Select("select * from Sha001 where `departure`=#{departure} and `arrival`=#{arrival} and `departureDate`=#{departureDate} and `seatNum` >= #{needs}")
